...
여기까지 하면 앱은 뜨지만 **로그인할 방법이 없다.** 카카오나 구글 중
하나만 해도 된다.
...
- **개발용 로그인**([`dev-sign-in.tsx`](../apps/web/src/features/auth/dev-sign-in.tsx))은
  프로덕션 빌드에서 렌더되지 않지만, 코드와 계정은 공개 저장소에 남아 있다.
  실제 사용자를 받기 전에 지우는 편이 낫다.
